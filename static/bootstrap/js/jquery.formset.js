    ...
    $.fn.formset = function(opts)
    {
            ...
            insertDeleteLink = function(row) {
                ...
                row.find('a.' + options.deleteCssClass).click(function() {
                    ...
                    if (del.length) {
                        ...
                        var forms = $('[id^=id_' + options.prefix +'][type="text"]').not('.formset-custom-template');
                        // var forms_hidden = $('[id^=id_' + options.prefix +'][type="hidden"]').filter('.form-control').not('.formset-custom-template');
                        var forms_hidden = $('[id^=id_' + options.prefix +'][type="hidden"]').filter('.hidden-formset').not('.formset-custom-template');
                        console.log(forms_hidden);
                        ...
                        for (var i=0, formCount=forms.length; i<formCount; i++) {
                            ...
                            applyExtraClasses(forms.eq(i), i);
                            forms.eq(i).each(function() {
                                if($(this).parents('.double-form').length == 1) {
                                    
                                    if(i%2==0){
                                        j=i/2
                                    }
                                    updateElementIndex($(this), options.prefix, j);
                                } else {
                                    updateElementIndex($(this), options.prefix, i);
                                }
                            });
                        }
                        for (var i=0, formCount=forms_hidden.length; i<formCount; i++) {
                            
                            applyExtraClasses(forms_hidden.eq(i), i);
                            forms_hidden.eq(i).each(function() {
                                updateElementIndex($(this), options.prefix, i);
                            });
                        }
                    }
        ...
        if ($$.length) {
            ...
            addButton.click(function() {
                ...
                // console.log(row[0].firstElementChild.id);
