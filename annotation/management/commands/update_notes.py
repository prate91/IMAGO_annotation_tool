...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for obj in a.all():
            body_nota = obj.data['Nota']['BodyNota']
            frammento_dante_ddp = obj.data['Nota']['FrammentoDanteDDP']

            # Espressioni regolare per togliere punteggiatura (.,:) e ec. 
            # seguito da punteggiatura all'inizio del body nota
            body_nota = re.sub(r'^[,.:]?\s+-?-?\s?', '', body_nota)
            body_nota = re.sub(r'^ec\.[,;:]?', '', body_nota)

            # Espressioni regolari per sostituire le parentisi angolari < e > 
            # con le virgolette &laquo; e &raquo; nel corpo della nota.
            # Evitare di sostituire quelle di apertura e chiusura dei tag html
            # < --> &laquo;
            body_nota = re.sub(r'<(?=\s?\w\w)', '&laquo;', body_nota)
            # > --> &raquo;
            body_nota = re.sub(r'(?<=\w[\.,;!\)\?\w])>', '&raquo;', body_nota)

            # Sostituzione del doppio trattino -- con il trattino lungo &mdash;
            body_nota = re.sub('--', '&mdash;', body_nota)

            # Sostituzione della codifica ddp per apici + ~ con tag sup <sup></sup>
        ...
        #                         # print(format_canto)
        #                         format_number_of_note  = "{0:0=3d}".format(number_of_note)
        #                         # print(format_number_of_note)

                                
        #                         if(re.search(r'\d+\s?-\s?\d+|\d+', note["Nota"]["OperaDante"]["Verso"])):
